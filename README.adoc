...
[source,bash]
----
kubectl apply -f deploy/role_olm.yaml
kubectl apply -f deploy/role_binding_olm.yaml
kubectl apply -f deploy/service_account_olm.yaml
kubectl apply -f deploy/crds/jaegertracing_v1_jaeger_crd.yaml
kubectl apply -f deploy/olm-catalog/jaeger-operator.csv.yaml
----
